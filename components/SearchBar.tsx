...
interface Props{
    ...
    value?: string;          // Optional value for the search input (not used correctly here)
    onChangeText?: (text: string) => void; // Optional function to handle text changes (not used correctly here)
}
    ...
    return (
        ...
        <View className='flex-row items-center  bg-dark-100 rounded-full px-5 py-4'>
            ...
            <TextInput
                ...
                value={value}                 // Input value is always empty (not controlled)
                onChangeText={onChangeText}   // No-op handler for text changes
                placeholderTextColor="white" // Color of the placeholder text
                className='flex-1 ml-3  text-white'     // Styling for input to expand and add margin
            />
        </View>
    )
}
